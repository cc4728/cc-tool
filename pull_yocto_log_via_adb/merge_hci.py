...
if __name__ == '__main__':
    ...
    REPORT_FILE = path+"\\"+ datetime.datetime.now().strftime('%M%S_%f')+'_merge_hci.cfa'
    #print("Save at => ",REPORT_FILE)
#remove repreat name
...
#    print(hci_list)
    for file in hci_list:
        ...
        if '\\' in file:
            ...
            name = file
        if not REPORT_FILE:
            REPORT_FILE = path+"\\"+ datetime.datetime.now().strftime('%M%S_%f')+'_'+name
        process_hci(file)
